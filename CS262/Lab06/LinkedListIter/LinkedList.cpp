...
namespace macs262_labs
{
	...
	void LinkedList<Item>::insertAtHead(const Item& entry){
		...
		head->next->prev = head;
		}//will make head point to a new node, since all inserts happen at the head, no prev pointer needs to be passed in the node constructor because it will always be NULL, which it defaults to.
